...
sub get_location_from_req ($) {
    ...
    if (my $pos = $req->param('pos')) {
        ...
        # Willcom does not send |geo| (?) but in Tokyo system.
        my $geo = $req->param('geo') || 'tokyo';
        if ($geo =~ /^[Tt][Oo][Kk][Yy][Oo]$/) {
            ($lat, $lon) = tokyo2wgs($lat, $lon);
        }
    } elsif ((my $js_lat = $req->param('js_lat') ||
              $req->param('geolat')) and
             (my $js_lon = $req->param('js_lon') ||
              $req->param('geolong'))) {
        $lat = $js_lat;
        $lon = $js_lon;
